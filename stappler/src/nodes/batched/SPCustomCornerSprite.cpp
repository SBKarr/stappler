...
bool CustomCornerSprite::init(uint32_t radius, float density) {
	...
	_contentSize = cocos2d::Size(_texContentSize.width * 2, _texContentSize.height * 2);
...
void CustomCornerSprite::onContentSizeDirty() {
...
void CustomCornerSprite::setDrawCenter(bool value) {
...
cocos2d::Rect CustomCornerSprite::textureRectForGrid(int i, int j) {
	if ((i == 0 || i == 2) && (j == 0 || j == 2)) {
		return cocos2d::Rect(0, 0, _texContentSize.width, _texContentSize.height);
	} else if (i == j && i == 1) {
		return cocos2d::Rect(0, 0, 1, 1);
	} else if (i == 0 || i == 2) {
		return cocos2d::Rect(0, 0, 1, _texContentSize.height);
	} else {
		return cocos2d::Rect(0, 0, _texContentSize.width, 1);
	}
}

cocos2d::Vec2 CustomCornerSprite::texturePositionForGrid(int i, int j, float csx, float csy, float expx, float expy) {
	cocos2d::Vec2 pos;
	if (i == 0) {
		pos.y = 0;
	} else if (i == 1) {
...
bool CustomCornerSprite::textureFlipX(int i, int j) {
	if (j == 0) {
		return true;
	} else {
		return false;
...
bool CustomCornerSprite::textureFlipY(int i, int j) {
	if (i == 0) {
		return false;
	} else {
		return true;
...
void CustomCornerSprite::updateSprites() {
