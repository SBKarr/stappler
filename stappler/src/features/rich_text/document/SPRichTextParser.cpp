...
namespace parser {
	...
	String resolveCssString(const String &origStr) {
		auto str = origStr;
		string::trim(str);

		auto first = origStr.find('(');
		auto last = origStr.rfind(')');
		if (first != String::npos && last != String::npos && first != last && last == str.length() - 1) {
	...
	style::MediaQuery::Query readMediaQuery(StringReader &s, const style::CssStringFunction &cb) {
		...
		while (s.is('(')) {
			...
			if (identifier.empty() && value.empty()) {
				...
				style::readMediaParameter(q.params, identifier, value, cb);
			}
